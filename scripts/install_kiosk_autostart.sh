...
Usage:
  $0 install [--yes] [--image-dir DIR] [--start] [--force] [--dry-run]
  $0 print-service [--image-dir DIR]
...
Safety:
  - install only writes/enables the service when --yes is passed.
  - install refuses to overwrite $SERVICE_PATH unless --force is passed.
...
if [ "$#" -gt 0 ]; then
  case "$1" in
    install|print-service|start|stop|restart|disable|status|logs|help|--help|-h)
      ACTION="$1"
      shift
      ;;
    --*)
